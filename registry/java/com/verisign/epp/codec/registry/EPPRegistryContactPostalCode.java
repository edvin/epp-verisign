 ...
 */
public class EPPRegistryContactPostalCode extends EPPRegistryMinMaxLength {
	/** DOCUMENT ME! */
	private static final long  serialVersionUID = 791449837693988719L;

	/** DOCUMENT ME! */
	public static final String ELM_NAME = "registry:pc";
	 ...
	 */
	public EPPRegistryContactPostalCode(Integer min, Integer max) {
		this();
		this.min     = min;
		this.max     = max;
	}
	 ...
	 * @param max DOCUMENT ME!
	 */
	public EPPRegistryContactPostalCode(int min, int max) {
	...
	protected void validateState() throws EPPEncodeException {
		...
		if (max.intValue() > 16) {
			throw new EPPEncodeException("Invalid state on "
					+ getClass().getName()
					+ ".encode: max should be less than or equal to 16");
		}
	}
}
