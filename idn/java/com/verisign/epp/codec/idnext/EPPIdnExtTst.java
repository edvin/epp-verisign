...
/**
 * Is a unit test of the com.verisign.epp.codec.idnext package. The unit test
 * will execute, gather statistics, and output the results of a test of each
 * com.verisign.epp.codec.idnext package concrete <code>EPPIdnExt</code>'s
 ...
 */
public class EPPIdnExtTst extends TestCase {
	/**
	...
	/**
	 * Creates a new EPPIdnExtTst object.
	 *
	...
	public void testDomainCreateCommandWithIdnLangExt() {
		...
		theDomainCommand = new EPPDomainCreateCmd("ABC-12345",
			"john.doe.name", new EPPAuthInfo("2fooBAR"));
	...
	// End EPPIdnExtTst.tearDown();

	/**
	 * JUNIT <code>suite</code> static method, which returns the tests
	 * associated with <code>EPPIdnExtTst</code>.
	 *
	 * @return DOCUMENT ME!
	 */
	public static Test suite() {
		...
		try {
			...
			Assert.fail("EPPCodecException adding EPPDomainMapFactory or EPPIdnExtFactory to EPPCodec: "
						+ e);
		}
        return suite;
	}

	// End EPPIdnExtTst.suite()
	...
	public static void main(String[] args) {
		...
		if (numThreads > 1) {
			...
			for (int i = 0; i < numThreads; i++) {
				TestThread thread =
					new TestThread(
								   "EPPIdnExtTst Thread " + i,
								   EPPIdnExtTst.suite());
				thread.start();
			}
		}
		else { // Single threaded mode.
			junit.textui.TestRunner.run(EPPIdnExtTst.suite());
		}
	}

	// End EPPIdnExtTst.main(String [])
	...
	// End EPPIdnExtTst.setNumIterations(long)
}
