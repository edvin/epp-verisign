...
import com.verisign.epp.codec.rgpext.*;
import com.verisign.epp.codec.rgppoll.*;


/**
 * The <code>RgpDomainHandler</code> class is a concrete implementation of the
 * abstract <code>com.verisign.epp.framework.EPPRgpDomainHandler</code> class. It
...
public class RgpDomainHandler extends EPPDomainHandler {
	...
	private static Logger cat =
		Logger.getLogger(
						 RgpDomainHandler.class.getName(),
	...
	public RgpDomainHandler() {
        ...
        try {
            EPPFactory.getInstance().addMapFactory(EPPRgpPollMapFactory.class
                                                  .getName());
        }
         catch (EPPCodecException e) {
            cat.error(
                      "Couldn't load the Map Factory associated with the EPPRgpPollMapFactory Mapping",
                      e);
            System.exit(1);
        }
    }
	 ...
	 * 		  <code>EPPRgpDomainHandler</code> This is assumed to be an instance
	 * 		  of SessionData here.
	 *
	...
	protected EPPEventResponse doDomainUpdate(EPPEvent aEvent, Object aData) {
       ...
       EPPDomainUpdateCmd theMessage =
        ...
        EPPRgpExtUpdate rgpUpdate = (EPPRgpExtUpdate)
            theMessage.getExtension(EPPRgpExtUpdate.class);
        ...
        if (rgpUpdate != null) {
			...
			if (rgpUpdate.getRestore().getOp().equals(EPPRgpExtRestore.REPORT)) {
				...
				cat.info("RgpDomainHandler.doDomainUpdate(): preData = "
						+ theReport.getPreData());
				cat.info("RgpDomainHandler.doDomainUpdate(): postData = "
						+ theReport.getPostData());
				cat.info("RgpDomainHandler.doDomainUpdate(): deleteTime = "
						+ theReport.getDeleteTime());
				cat.info("RgpDomainHandler.doDomainUpdate(): restoreTime = "
						+ theReport.getRestoreTime());
				...
				thePollMsg.setReportDueDate(new Date(
				...
				try {
					EPPPollQueueMgr.getInstance().put(null,
							EPPRgpPollMapFactory.NS, thePollMsg, null);
				}
				catch (EPPPollQueueException ex) {
					cat.error("doDomainUpdate: Error putting message ["
							+ thePollMsg + "]");
