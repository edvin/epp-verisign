 ...
 * should be called prior to instantiating an instance of <code>EPPPlainProxyClientSocket</code> 
...
public class EPPPlainProxyClientSocket extends EPPPlainClientSocket {
	...
	private static Logger cat = Logger.getLogger(
			EPPPlainProxyClientSocket.class.getName(), EPPCatFactory
					.getInstance().getFactory());

	/**
	 * List of {@link EPPProxyServer} instances creating from parsing the
	...
	public EPPPlainProxyClientSocket() throws EPPConException {
		...
		cat.debug("EPPPlainProxyClientSocket(): start");

		// Get the proxy server properties
		try {
			// Locator not previously instantiated?
			if (EPPPlainProxyClientSocket.locator == null) {
				...
				EPPPlainProxyClientSocket.locator = (EPPProxyServersLocator) locatorClass
						.newInstance();
			}

			this.proxyServers = EPPPlainProxyClientSocket.locator
					.getProxyServers();
		...
		cat.debug("EPPPlainProxyClientSocket(): exit");
	}
	...
	public void initialize() throws EPPConException {
		...
		try {
			super.setInputStream(theSocket.getInputStream());
			super.setOutputStream(theSocket.getOutputStream());
			super.setSocket(theSocket);
	 ...
	 * <code>EPPPlainProxyClientSocket</code>. The locator is a static / global
	 ...
	 * <code>EPPPlainProxyClientSocket</code> is instantiated, the default locator
	 ...
	 * <code>EPPPlainProxyClientSocket</code> is instantiated indirectly in
