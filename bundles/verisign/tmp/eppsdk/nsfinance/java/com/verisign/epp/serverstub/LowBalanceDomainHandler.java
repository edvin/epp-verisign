...
/**
 * The <code>LowBalanceDomainHandler</code> class is a concrete implementation of the
...
public class LowBalanceDomainHandler extends EPPDomainHandler {
	...
	private static Logger cat =
		Logger.getLogger(
						 LowBalanceDomainHandler.class.getName(),
	...
	public LowBalanceDomainHandler() {
        ...
        try {
            EPPFactory.getInstance().addMapFactory(EPPLowBalancePollMapFactory.class
                                                  .getName());
        }
         catch (EPPCodecException e) {
            cat.error(
                      "Couldn't load the Map Factory associated with the EPPLowBalancePollMapFactory Mapping",
                      e);
            System.exit(1);
        }
    }
	 ...
	 * 		  <code>EPPLowBalanceDomainHandler</code> This is assumed to be an instance
	 * 		  of SessionData here.
	 *
	 ...
	 * @param aEvent The <code>EPPEvent</code> that is being handled
	 * @param aData Any data that a Server needs to send to this
	 * 		  <code>EPPLowBalanceDomainHandler</code>
	 *
	 * @exception EPPHandleEventException Thrown if an error condition occurs.
	 * 			  It must contain an <code>EPPEventResponse</code>
	 */
