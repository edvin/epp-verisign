 ...
 * </p>
 * <p>
 * Description: The EPPRelatedDomainExtRenewResp object represents the
 * collection of domains that must be created atomically. As such it is composed
 * of a collection of {@link EPPRelatedDomainExtDomainData} objects. <br/>
 * As XML, it is represented by a <relDom:renData> element containing a number
 ...
 * @version 1.0
 */
public class EPPRelatedDomainExtRenewResp implements EPPCodecComponent {
	...
	private static Logger cat = Logger.getLogger(
			EPPRelatedDomainExtRenewResp.class.getName(), EPPCatFactory
					.getInstance()
					.getFactory() );
	...
	/**
	 * List of domain names represented by the {@link EPPRelatedDomainExtDomainData}
	 ...
	 * 
	 * @param aDomains List of domains renewed
	 */
	public EPPRelatedDomainExtRenewResp(
	...
	public Object clone () throws CloneNotSupportedException {
		final EPPRelatedDomainExtRenewResp theClone = new EPPRelatedDomainExtRenewResp();
	...
	public void decode ( final Element aElement ) throws EPPDecodeException {
		...
		try {
			...
			throw new EPPDecodeException(
					"Invalid state on EPPRelatedDomainExtRenewResp.decode: " + e );
		}
	}
	 ...
	 * represented by {@link EPPRelatedDomainExtDomainData} to given DOM Document
	 * 
	 * @param aDocument
	...
	public Element encode ( final Document aDocument ) throws EPPEncodeException {
		...
		if ( aDocument == null ) {
			throw new EPPEncodeException( "aDocument is null"
					+ " in EPPRelatedDomainExtRenewResp.encode(Document)" );
		}
		...
		try {
			...
			cat
					.error( "EPPRelatedDomainExtRenewResp.encode(): Invalid state on encode: "
							+ e );
			throw new EPPEncodeException(
					"EPPRelatedDomainExtRenewResp invalid state: " + e );
		}
	...
	public boolean equals ( final Object aObj ) {
		...
		if ( !(aObj instanceof EPPRelatedDomainExtRenewResp) ) {
			return false;
		}

		final EPPRelatedDomainExtRenewResp theComp =
				(EPPRelatedDomainExtRenewResp) aObj;

		if ( !EPPUtil.equalLists( this.domains, theComp.domains ) ) {
			cat
					.error( "EPPRelatedDomainExtRenewResp.equals(): related domains not equal" );
			return false;
		}
	 ...
	 * {@link EPPRelatedDomainExtDomainData} to the list of related domains renewed.
	 * 
	 * @param aDomain Related domain renewed to add to the list.
	 */
	...
	/**
	 * Returns the list of renewed domains represented by
	...
	/**
	 * Sets the list of renewed domains represented by
	 * {@link EPPRelatedDomainExtDomainData}
	 * 
	 * @param aDomains
	...
	/**
	 * Validate the state of the <code>EPPRelatedDomainExtRenewResp</code>
	...
	private void validateState () throws EPPCodecException {
		if ( (this.domains == null) || (this.domains.size() == 0) ) {
			throw new EPPCodecException(
					"EPPRelatedDomainExtRenewResp contains no  elements." );
		}

		for (EPPRelatedDomainExtDomainData domain : this.domains) {
			if ( domain == null ) {
				throw new EPPCodecException(
						"EPPRelatedDomainExtRenewResp: contains null EPPRelatedDomainExtDomainData element." );
			}
